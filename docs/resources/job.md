...
* `control_run_state` - (Optional) (Bool) If true, the Databricks provider will stop and start the job as needed to ensure that the active run for the job reflects the deployed configuration. For non-continuous jobs, the provider will stop the current active run if available and start a new run. For continuous jobs, the provider respects the `pause_status` by stopping the current active run.
