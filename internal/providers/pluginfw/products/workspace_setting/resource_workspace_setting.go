...
const resourceName = "workspace_setting"
...
func (r *SettingResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	...
	resp.Schema = schema.Schema{
		Description: "Terraform schema for Databricks workspace_setting",
		Attributes:  attrs,
		Blocks:      blocks,
	}
}
...
func (r *SettingResource) update(ctx context.Context, plan settingsv2_tf.Setting, diags *diag.Diagnostics, state *tfsdk.State) {
	...
	if err != nil {
		diags.AddError("failed to update workspace_setting", err.Error())
		return
	}
