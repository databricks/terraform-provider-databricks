...
-> **Note** Please switch to [databricks_storage_credential](storage_credential.md) with Unity Catalog to manage storage credentials, which provides better and faster way for managing credential security. `databricks_storage_credential` resource *doesn't require a technical cluster to perform operations*. `databricks_instance_profile` will be removed, once Unity Catalog is Generally Available.
