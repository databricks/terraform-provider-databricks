...
func typeToSchema(ctx context.Context, v reflect.Value) NestedBlockObject {
	...
	for _, field := range tfreflect.ListAllFields(v) {
		...
		attr := getAttribute(ctx, v, field, structTag)
...
func getAttribute(ctx context.Context, v reflect.Value, field tfreflect.Field, structTag structTag) AttributeBuilder {
	typeField := field.StructField
	fieldName := typeField.Tag.Get("tfsdk")
	...
	switch value.(type) {
	case types.Bool:
		return BoolAttributeBuilder{}
	case types.Int64:
		return Int64AttributeBuilder{}
	case types.Float64:
		return Float64AttributeBuilder{}
	case types.String:
		return StringAttributeBuilder{}
	case types.List, types.Map, types.Object:
		...
		case reflect.TypeOf(types.Bool{}), reflect.TypeOf(types.Int64{}), reflect.TypeOf(types.Float64{}), reflect.TypeOf(types.String{}):
			...
			switch value.(type) {
			case types.List:
				return ListAttributeBuilder{ElementType: containerType.ElementType()}
			case types.Map:
				return MapAttributeBuilder{ElementType: containerType.ElementType()}
			}
		default:
			...
			case types.Map:
				return MapNestedAttributeBuilder{
					NestedObject: nestedSchema.ToNestedAttributeObject(),
				}
			case types.Object:
	...
	panic(fmt.Errorf("unreachable code reached. %s", common.TerraformBugErrorMessage))
}
