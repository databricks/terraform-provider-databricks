...
func DataSourceBranches() datasource.DataSource {
...
func (r *BranchesDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	attrs, blocks := tfschema.DataSourceStructToSchemaMap(ctx, BranchesData{}, nil)
...
func (r *BranchesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	response, err := client.Postgres.ListBranchesAll(ctx, listRequest)
	if err != nil {
		resp.Diagnostics.AddError("failed to list postgres_branches", err.Error())
		return
	}
	...
	for _, item := range response {
		...
		resp.Diagnostics.Append(converters.GoSdkToTfSdkStruct(ctx, item, &branch)...)
		if resp.Diagnostics.HasError() {
			return
		}
