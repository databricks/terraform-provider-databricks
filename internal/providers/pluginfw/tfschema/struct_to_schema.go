...
func typeToSchema(ctx context.Context, v reflect.Value) NestedBlockObject {
	...
	for _, field := range tfreflect.ListAllFields(v) {
		...
		case types.List, types.Map, types.Object:
			...
			objectTypable, ok := v.Interface().(tfcommon.ObjectTypable)
			if !ok {
				panic(fmt.Errorf("object type not provided for type: %T. %s", v.Interface(), common.TerraformBugErrorMessage))
			}
			objectType := objectTypable.ToObjectType(ctx)
			...
			case reflect.TypeOf(types.Bool{}), reflect.TypeOf(types.Int64{}), reflect.TypeOf(types.Float64{}), reflect.TypeOf(types.String{}):
				switch value.(type) {
				case types.List:
					scmAttr[fieldName] = ListAttributeBuilder{ElementType: attrType}
				case types.Map:
					scmAttr[fieldName] = MapAttributeBuilder{ElementType: attrType}
				case types.Object:
