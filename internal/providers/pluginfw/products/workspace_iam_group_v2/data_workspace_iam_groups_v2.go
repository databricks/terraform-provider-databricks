...
const dataSourcesName = "workspace_iam_groups_v2"
...
type GroupsData struct {
	WorkspaceIamV2 types.List `tfsdk:"groups"`
...
func (r *GroupsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var listRequest iamv2.ListGroupsProxyRequest
	...
	response, err := client.WorkspaceIamV2.ListGroupsProxyAll(ctx, listRequest)
	if err != nil {
		resp.Diagnostics.AddError("failed to list workspace_iam_groups_v2", err.Error())
		return
	}
	...
	for _, item := range response {
		...
		group.ProviderConfigData = config.ProviderConfigData

		results = append(results, group.ToObjectValue(ctx))
	}

	config.WorkspaceIamV2 = types.ListValueMust(GroupData{}.Type(ctx), results)
