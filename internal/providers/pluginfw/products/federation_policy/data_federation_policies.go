...
const dataSourcesName = "federation_policies"
...
type FederationPoliciesList struct {
	...
	FederationPolicies types.List `tfsdk:"policies"`
}
...
func (r *FederationPoliciesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	response, err := client.FederationPolicies.ListAll(ctx, listRequest)
	if err != nil {
		resp.Diagnostics.AddError("failed to list federation_policies", err.Error())
		return
	}
	...
	newState.FederationPolicies = types.ListValueMust(sharing_tf.FederationPolicy{}.Type(ctx), policies)
