...
type SqlColumnInfo struct {
	...
	Identity string `json:"identity,omitempty" tf:"default:false"`
...
func (ci *SqlColumnInfo) serializeIdentityColumnInfo() string {
	var identityType string
	switch ci.Identity {
	case "always":
		identityType = "BIGINT GENERATED ALWAYS AS IDENTITY"
	case "default":
		identityType = "BIGINT GENERATED BY DEFAULT AS IDENTITY"
	}
	return identityType
}
...
func (ti *SqlTableInfo) serializeColumnInfo(col SqlColumnInfo) string {
	...
	if col.Identity != "false" {
		colType = col.serializeIdentityColumnInfo()
	} else {
		colType = col.Type
	}
