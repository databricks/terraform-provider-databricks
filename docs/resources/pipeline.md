...
* `preserve_tables_on_delete` - (Optional) When `true`, destroying this resource deletes the pipeline without dropping tables owned by the pipeline (the delete API is called with `cascade` disabled). When `false` (default), those tables are dropped as part of deletion. Requires `catalog` to be set — the cascade flag is only supported for Unity Catalog-managed pipelines. This only affects `terraform destroy` (and failed-create cleanup); it does not change pipeline runtime behavior.
