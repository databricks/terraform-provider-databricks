...
const dataSourcesName = "account_federation_policies"
...
func (r *FederationPoliciesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var listRequest oauth2.ListAccountFederationPoliciesRequest
	...
	response, err := client.FederationPolicy.ListAll(ctx, listRequest)
	if err != nil {
		resp.Diagnostics.AddError("failed to list account_federation_policies", err.Error())
		return
	}
