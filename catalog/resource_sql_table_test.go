...
func TestResourceSqlTableCreateStatement_External(t *testing.T) {
	...
	stmt, err := ti.buildTableCreateStatement()
...
func TestResourceSqlTableCreateStatement_NotPrimaryOrForeignKeyConstraint(t *testing.T) {
	ti := &SqlTableInfo{
		Name:                  "bar",
		CatalogName:           "main",
		SchemaName:            "foo",
		...
		Constraints: []ConstraintInfo{
			...
			},
			{
				Name:         "fk",
				Type:         "CHECK",
				CheckFormula: "length(country_code) == 2",
			},
		},
	}
	_, err := ti.buildTableCreateStatement()
	assert.Error(t, err, "constraint of type CHECK is not supported for CREATE TABLE statement")
}
