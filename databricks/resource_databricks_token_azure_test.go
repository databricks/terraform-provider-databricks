...
func TestAccAzureTokenResource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccAzureCheckTokenResourceDestroy,
		Steps: []resource.TestStep{
			{
				// use a dynamic configuration with the random name from above
				Config: testAccAzureTokenResource(rComment),
				// compose a basic test, checking both remote and local values
				Check: resource.ComposeTestCheckFunc(
					// query the API to retrieve the tokenInfo object
					testAccAzureCheckTokenResourceExists("databricks_token.my-token", &tokenInfo, t),
					// verify remote values
					testAccAzureCheckTokenValues(&tokenInfo, rComment),
					// verify local values
...
func testAccAzureCheckTokenResourceDestroy(s *terraform.State) error {
...
func testAccAzureCheckTokenValues(tokenInfo *model.TokenInfo, comment string) resource.TestCheckFunc {
...
func testAccAzureCheckTokenResourceExists(n string, tokenInfo *model.TokenInfo, t *testing.T) resource.TestCheckFunc {
	return func(s *terraform.State) error {
...
// testAccAzureTokenResource returns an configuration for an Example Widget with the provided name
func testAccAzureTokenResource(comment string) string {
	return fmt.Sprintf(`
