...
* `user_agent_extra` - (optional, environment variable `DATABRICKS_USER_AGENT_EXTRA`) a whitespace-separated list of products to append to the `User-Agent` header on every Databricks REST API request, e.g. for usage attribution of Terraform modules built on top of the provider. Each product string must follow [RFC 9110](https://www.rfc-editor.org/rfc/rfc9110#name-user-agent), include a version, and can't contain comments, e.g. `user_agent_extra = "databricks-sra/1.2.0 my-project/0.0.1"`. Products configured in the provider block and in the environment variable are merged, with duplicates removed.
