...
import (
	...
	sql_api "github.com/databrickslabs/terraform-provider-databricks/sql/api"
...
func dbsqlListObjects(ic *importContext, path string) (events []map[string]interface{}, err error) {
	...
	for len(events) < totalCount {
		...
		err := ic.Client.Get(ic.Context, path,
			map[string]interface{}{"page_size": 100, "page": page},
			&listResponse)
		if err != nil {
			return nil, err
		}
