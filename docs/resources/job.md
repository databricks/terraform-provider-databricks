...
* `always_running` - (Optional) (Bool) Indicate whether a new run of the job shall be triggered when applying the template. This field can be used with long-running applications, for example applications using Spark Streaming or continuous jobs, to ensure that a run using the new job configuration is immediately started. If there is an existing job run, it is cancelled before the new job run is started. The default behavior is to not trigger a new run.
