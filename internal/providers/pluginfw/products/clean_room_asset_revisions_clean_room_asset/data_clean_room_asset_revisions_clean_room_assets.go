...
const dataSourcesName = "clean_room_asset_revisions_clean_room_assets"
...
func (r *CleanRoomAssetsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var listRequest cleanrooms.ListCleanRoomAssetRevisionsRequest
	resp.Diagnostics.Append(converters.TfSdkToGoSdkStruct(ctx, config, &listRequest)...)
	if resp.Diagnostics.HasError() {
		return
	}

	response, err := client.CleanRoomAssetRevisions.ListAll(ctx, listRequest)
	if err != nil {
		resp.Diagnostics.AddError("failed to list clean_room_asset_revisions_clean_room_assets", err.Error())
		return
	}
