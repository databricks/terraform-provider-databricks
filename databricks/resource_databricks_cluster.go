...
func resourceClusterCreate(d *schema.ResourceData, m interface{}) error {
	...
	if len(libraryList.Libraries) > 0 {
		err = client.Libraries().Install(libraryList)
		if err != nil {
			return err
		}
		_, err := waitForLibrariesInstalled(client.Libraries(), d.Id())
		if err != nil {
...
func resourceClusterRead(d *schema.ResourceData, m interface{}) error {
	client := m.(*service.DatabricksClient)
	clusterInfo, err := client.Clusters().Get(d.Id())
	if ae, ok := err.(service.APIError); ok && ae.IsMissing() {
	...
	err = util.StructToData(clusterInfo, clusterSchema, d)
	if err != nil {
		return err
	}
	libsClusterStatus, err := waitForLibrariesInstalled(client.Libraries(), d.Id())
	if err != nil {
		// do we consider failed installation as missing cluster?...
		return err
	}
	libList := libsClusterStatus.ToLibraryList()
	return util.StructToData(libList, clusterSchema, d)
}

func waitForLibrariesInstalled(
	libraries service.LibrariesAPI, clusterID string) (result *model.ClusterLibraryStatuses, err error) {
...
func legacyReadLibraryListFromData(d *schema.ResourceData) (cll model.ClusterLibraryList) {
...
func resourceClusterDelete(d *schema.ResourceData, m interface{}) error {
	return m.(*service.DatabricksClient).Clusters().PermanentDelete(d.Id())
}
