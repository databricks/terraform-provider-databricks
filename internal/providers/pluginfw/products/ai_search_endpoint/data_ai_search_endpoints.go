...
const dataSourcesName = "ai_search_endpoints"
...
type EndpointsData struct {
	AiSearch types.List `tfsdk:"endpoints"`
...
func (r *EndpointsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var listRequest aisearch.ListEndpointsRequest
	...
	response, err := client.AiSearch.ListEndpointsAll(ctx, listRequest)
	if err != nil {
		resp.Diagnostics.AddError("failed to list ai_search_endpoints", err.Error())
		return
	}
	...
	config.AiSearch = types.ListValueMust(EndpointData{}.Type(ctx), results)
