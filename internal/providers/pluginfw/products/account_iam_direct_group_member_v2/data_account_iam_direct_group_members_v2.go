...
const dataSourcesName = "account_iam_direct_group_members_v2"
...
func (r *DirectGroupMembersDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var listRequest iamv2.ListDirectGroupMembersRequest
	...
	response, err := client.IamV2.ListDirectGroupMembersAll(ctx, listRequest)
	if err != nil {
		resp.Diagnostics.AddError("failed to list account_iam_direct_group_members_v2", err.Error())
		return
	}
	...
	config.AccountIamV2 = types.ListValueMust(DirectGroupMemberData{}.Type(ctx), results)
