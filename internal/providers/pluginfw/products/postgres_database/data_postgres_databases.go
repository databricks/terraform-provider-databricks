...
const dataSourcesName = "postgres_databases"
...
func (DatabasesData) GetComplexFieldTypes(context.Context) map[string]reflect.Type {
	return map[string]reflect.Type{
		"databases": reflect.TypeOf(DatabaseData{}),
		"provider_config": reflect.TypeOf(ProviderConfigData{}),
		
	}
}

func (m DatabasesData) ApplySchemaCustomizations(attrs map[string]tfschema.AttributeBuilder) map[string]tfschema.AttributeBuilder {attrs["parent"] = attrs["parent"].SetRequired()
attrs["page_size"] = attrs["page_size"].SetOptional()

	attrs["databases"] = attrs["databases"].SetComputed()
...
func (r *DatabasesDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	attrs, blocks := tfschema.DataSourceStructToSchemaMap(ctx, DatabasesData{}, nil)
...
func (r *DatabasesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var config DatabasesData
	resp.Diagnostics.Append(req.Config.Get(ctx, &config)...)
	if resp.Diagnostics.HasError() {
		return
	}

	var listRequest postgres.ListDatabasesRequest
	...
	response, err := client.Postgres.ListDatabasesAll(ctx, listRequest)
	if err != nil {
		resp.Diagnostics.AddError("failed to list postgres_databases", err.Error())
		return
	}
	...
	for _, item := range response {
		...
		resp.Diagnostics.Append(converters.GoSdkToTfSdkStruct(ctx, item, &database)...)
		if resp.Diagnostics.HasError() {
			return
		}
		database.ProviderConfigData = config.ProviderConfigData
		
		results = append(results, database.ToObjectValue(ctx))
	}

	config.Postgres = types.ListValueMust(DatabaseData{}.Type(ctx), results)
