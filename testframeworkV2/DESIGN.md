...
### 12.9 Makefile shortcut

`testframeworkV2/Makefile` wraps the existing CLI + `go test` entry points behind compact targets so day-to-day fixture runs are a one-liner. Adding the Makefile is purely additive — no new dependencies, no behaviour change, no API extension. Every target dispatches to a command the user can also invoke directly.

```
make test <path>     Run a single test (delegates to `go run ./cmd/tfv2 run <path>`)
make test-all        Run every fixture (wraps `TFV2_RUN=1 go test -run TestFixtures -v ./...`)
make unit            Unit tests only — no cloud auth required (`go test ./internal/... ./cmd/...`)
make build           Build `./tfv2` binary (`go build -o tfv2 ./cmd/tfv2`)
make clean           Remove the built binary
make help            Default target — print the usage banner
```

`make test <path>` uses the `%: @:` catch-all idiom + `MAKECMDGOALS` filter to accept a positional path argument. The user types `make test issues-repro/issue_5672/`; Make sees two goals (`test`, `issues-repro/issue_5672/`); the catch-all silently no-ops the second; the `test` recipe extracts the path via `filter-out test,$(MAKECMDGOALS)` and shells out to the CLI. Trailing slash is tolerated (the CLI normalises via `filepath.Abs`). Empty `make test` exits 1 with a usage hint.

`make test-all` sets `TFV2_RUN=1` so the env-gate from §12.7 is satisfied automatically — the test sweep would skip every fixture without it, defeating the point of the target. `make unit` deliberately omits `TFV2_RUN`, so it's safe to run on a developer laptop without cloud auth: only the unit tests under `internal/...` and `cmd/...` execute.

The README "Quickstart" and CONTRIBUTING "Step 4 — Run locally" now lead with `make test <path>` as the primary invocation form. CLI (`go run ./cmd/tfv2 run`) and `go test` (`TFV2_RUN=1 go test -run TestFixtures/...`) forms remain documented as alternatives — both produce identical output, and IDEs configured to shell out via `make` versus `go test` versus the CLI binary are all equally supported.
