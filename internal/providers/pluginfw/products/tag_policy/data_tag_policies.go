...
type TagPoliciesList struct {
	tags_tf.ListTagPoliciesRequest
	TagPolicies types.List `tfsdk:"tag_policies"`
}

func (c TagPoliciesList) ApplySchemaCustomizations(attrs map[string]tfschema.AttributeBuilder) map[string]tfschema.AttributeBuilder {
	attrs["tag_policies"] = attrs["tag_policies"].SetComputed()
	return attrs
}

func (TagPoliciesList) GetComplexFieldTypes(context.Context) map[string]reflect.Type {
...
func (r *TagPoliciesDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	attrs, blocks := tfschema.DataSourceStructToSchemaMap(ctx, TagPoliciesList{}, nil)
