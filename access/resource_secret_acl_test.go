...
func TestSecretsScopesAclsIntegration_ScopeWithSlash(t *testing.T) {
	cloud := os.Getenv("CLOUD_ENV")
	if cloud == "" {
		t.Skip("Acceptance tests skipped unless env 'CLOUD_ENV' is set")
	}
	...
	testScope := "myapplication/branch"
	testKey := "my-test-key"
	testSecret := "my-test-secret"
	initialManagePrincipal := ""
	// TODO: on random group
...
func TestResourceSecretACLRead_ScopeWithSlash(t *testing.T) {
	d, err := qa.ResourceFixture{
		Fixtures: []qa.HTTPFixture{
			{
				Method:   "GET",
	...
	assert.NoError(t, err, err)
	assert.Equal(t, "myapplication/branch|||something", d.Id(), "Id should not be empty")
	assert.Equal(t, "CAN_MANAGE", d.Get("permission"))
	assert.Equal(t, "something", d.Get("principal"))
	assert.Equal(t, "myapplication/branch", d.Get("scope"))
}
...
func TestResourceSecretACLRead_ScopeWithSlash_NotFound(t *testing.T) {
	qa.ResourceFixture{
		Fixtures: []qa.HTTPFixture{
			{
				Method:   "GET",
...
func TestResourceSecretACLRead_ScopeWithSlash_Error(t *testing.T) {
	d, err := qa.ResourceFixture{
		Fixtures: []qa.HTTPFixture{
			{
				Method:   "GET",
	...
	qa.AssertErrorStartsWith(t, err, "Internal error happened")
	assert.Equal(t, "myapplication/branch|||something", d.Id(), "Id should not be empty for error reads")
}
...
func TestResourceSecretACLCreate_ScopeWithSlash_Error(t *testing.T) {
	d, err := qa.ResourceFixture{
		Fixtures: []qa.HTTPFixture{
			{ // read log output for better stub url...
				Method:   "POST",
...
func TestResourceSecretACLDelete_ScopeWithSlash(t *testing.T) {
	d, err := qa.ResourceFixture{
		Fixtures: []qa.HTTPFixture{
			{
				Method:   "POST",
