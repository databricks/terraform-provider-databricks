...
* Databricks must have access to 2 secondary IP ranges, one between /21 to /9 for cluster pods, and one between /27 to /16 for cluster services.
...
* `gcp_network_info` - (GCP only) a block consists of Google Cloud specific information for this network, for example the VPC ID, subnet ID, and secondary IP ranges. It has the following fields:
  ...
  * `pod_ip_range_name` (Deprecated) - This field is now ignored and will be removed in a future version of the Terraform Provider for Databricks. The name of the secondary IP range for pods. A Databricks-managed GKE cluster uses this IP range for its pods. This secondary IP range can only be used by one workspace.
  * `service_ip_range_name` (Deprecated) - This field is now ignored and will be removed in a future version of the Terraform Provider for Databricks. The name of the secondary IP range for services. A Databricks-managed GKE cluster uses this IP range for its services. This secondary IP range can only be used by one workspace.
