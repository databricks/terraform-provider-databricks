...
func TestGrantsForIdChange(t *testing.T) {
	d, err := qa.ResourceFixture{
			...
			},
			{
				...
				ExpectedRequest: catalog.UpdatePermissions{
					Changes: []catalog.PermissionsChange{
						{
							Principal: "me",
							Add:       []catalog.Privilege{"CREATE_EXTERNAL_TABLE"},
						},
					},
				},
			},
			{
				...
				Response: catalog.PermissionsList{
					PrivilegeAssignments: []catalog.PrivilegeAssignment{
						{
							Principal:  "me",
							Privileges: []catalog.Privilege{"CREATE_EXTERNAL_TABLE"},
						},
					},
				},
			},
		...
		Resource:    ResourceGrants(),
		Update:      true,
		ID:          "external_location/old.location",
		RequiresNew: true,
		InstanceState: map[string]string{
			"external_location": "old.location",
		},
		HCL: `
		external_location = "new.location"
	...
	assert.Equal(t, "external_location/new.location", d.Id())
}
