...
const resourceName = "federation_policy"
...
func (r *FederationPolicyResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	...
	resp.Schema = schema.Schema{
		Description: "Terraform schema for Databricks federation_policy",
		Attributes:  attrs,
		Blocks:      blocks,
	}
}
...
func (r *FederationPolicyResource) update(ctx context.Context, plan sharing_tf.FederationPolicy, diags *diag.Diagnostics, state *tfsdk.State) {
	...
	response, err := client.FederationPolicies.Update(ctx, updateRequest)
	if err != nil {
		diags.AddError("failed to update federation_policy", err.Error())
		return
	}
...
func (r *FederationPolicyResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	response, err := client.FederationPolicies.Create(ctx, sharing.CreateFederationPolicyRequest{Policy: federation_policy})
	if err != nil {
		resp.Diagnostics.AddError("failed to create federation_policy", err.Error())
		return
	}
...
func (r *FederationPolicyResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	response, err := client.FederationPolicies.GetFederationPolicy(ctx, readRequest)
	if err != nil {
		...
		resp.Diagnostics.AddError("failed to get federation_policy", err.Error())
		return
	}
...
func (r *FederationPolicyResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	err := client.FederationPolicies.Delete(ctx, deleteRequest)
	if err != nil && !apierr.IsMissing(err) {
		resp.Diagnostics.AddError("failed to delete federation_policy", err.Error())
		return
	}
}
