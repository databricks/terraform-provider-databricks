...
const resourceName = "request_for_access"
...
func (r *AccessRequestDestinationResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	...
	resp.Schema = schema.Schema{
		Description: "Terraform schema for Databricks request_for_access",
		Attributes:  attrs,
		Blocks:      blocks,
	}
}
...
func (r *AccessRequestDestinationResource) update(ctx context.Context, plan catalog_tf.AccessRequestDestinations, diags *diag.Diagnostics, state *tfsdk.State) {
	...
	response, err := client.RequestForAccess.UpdateAccessRequestDestinations(ctx, updateRequest)
	if err != nil {
		diags.AddError("failed to update request_for_access", err.Error())
		return
	}
...
func (r *AccessRequestDestinationResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	response, err := client.RequestForAccess.GetAccessRequestDestinations(ctx, readRequest)
	if err != nil {
		...
		resp.Diagnostics.AddError("failed to get request_for_access", err.Error())
		return
	}
