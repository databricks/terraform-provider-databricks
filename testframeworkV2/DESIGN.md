...
| 4 | `fixed_on_local`    | `local`   | PASS | The **real** code fix on the current branch (PRs #5667 + #5666) holds |

Step 4 is the one that genuinely exercises the `local` build pathway — the most interesting capability of the framework — and is the only step that proves PR #5667's `common.AccountData` migration works.
...
**Step 4** uses the current branch via `local` build (synthetic version `99.0.0-local`). The current branch contains PR #5667 (`ab834c38` in `git log`), which migrates `databricks_mws_workspaces` from `common.DataResource` → `common.AccountData`. `AccountData` doesn't inject `provider_config` into the schema, so the post-Read hook never fires. Plan succeeds. **This step is the one that proves the real fix works.**
