# NEXT CHANGELOG

## Release v1.86.0

### Breaking Changes

### New Features and Improvements

* Added `databricks_groups` data source to retrieve multiple groups matching a filter criteria ([#XXXX](https://github.com/databricks/terraform-provider-databricks/pull/XXXX)).

### Bug Fixes

* Corrected optional fields in `databricks_mws_ncc_private_endpoint_rule`([#4856](https://github.com/databricks/terraform-provider-databricks/pull/4856)).

### Documentation

* Refreshed `databricks_job` documentation ([#4861](https://github.com/databricks/terraform-provider-databricks/pull/4861)).

### Exporter

### Internal Changes
