...
* Fix plan-time `workspace_id` validation firing on values the user did not type. The SDKv2 `NamespaceValidateWorkspaceID` and Plugin Framework `ValidateWorkspaceID` validators previously fell back to the auto-resolved provider-level `workspace_id` and made API calls against it, causing failures for limited-permission service principals ([#5668](https://github.com/databricks/terraform-provider-databricks/issues/5668)) and breaking the bootstrap pattern where `provider_config.workspace_id = databricks_mws_workspaces.this.workspace_id` is unknown at plan time. The validators now defer when the user did not explicitly write `provider_config.workspace_id` (null / unknown / empty); apply-time client construction still surfaces actionable errors via `GetWorkspaceClientForUnifiedProvider`.
