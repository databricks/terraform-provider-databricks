...
type CustomTemplatesList struct {
	apps_tf.ListCustomTemplatesRequest
	AppsSettings types.List `tfsdk:"templates"`
}

func (c CustomTemplatesList) ApplySchemaCustomizations(attrs map[string]tfschema.AttributeBuilder) map[string]tfschema.AttributeBuilder {
	attrs["templates"] = attrs["templates"].SetComputed()
	return attrs
}

func (CustomTemplatesList) GetComplexFieldTypes(context.Context) map[string]reflect.Type {
...
func (r *CustomTemplatesDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	attrs, blocks := tfschema.DataSourceStructToSchemaMap(ctx, CustomTemplatesList{}, nil)
...
func (r *CustomTemplatesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var config CustomTemplatesList
	...
	var newState CustomTemplatesList
