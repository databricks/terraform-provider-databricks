...
---
# databricks_ai_gateway_model_service Resource
...
* `config` (ModelServiceConfig, optional) - Operational configuration: destinations, routing, rate limits, inference
  table. Required on CreateModelService; on UpdateModelService it is
  required only when `config` (or a `config.*` subpath) appears in
  `update_mask`
* `owner` (string, optional) - The owner of the model service. Write-only; read owner via effective_owner
...
### ModelServiceConfig
* `inference_table` (InferenceTableConfig, optional) - Inference table config for payload logging
* `rate_limits` (list of RateLimit, optional) - Rate limits applied to requests routed through this model service
* `routing` (ModelServiceConfigRoutingConfig, optional) - Routing configuration: destinations, routing strategy, and fallback

### ModelServiceConfigDestinationConfig
* `destination_type` (string, required) - Backing-model category. Determines which oneof variant is populated. Possible values are: `DESTINATION_TYPE_EXTERNAL_FOUNDATION_MODEL`, `DESTINATION_TYPE_PAY_PER_TOKEN_FOUNDATION_MODEL`, `DESTINATION_TYPE_PROVISIONED_THROUGHPUT_FOUNDATION_MODEL`
* `name` (string, required) - User-facing label for this destination, used in routing references
* `external_model_config` (ModelServiceConfigExternalModelConfig, optional)
* `pay_per_token_config` (ModelServiceConfigPayPerTokenConfig, optional)
* `provisioned_throughput_config` (ModelServiceConfigProvisionedThroughputConfig, optional)
* `traffic_percentage` (integer, optional) - Share of traffic sent to this destination, 0-100. Optional on fallback
  destinations; see FallbackConfig

### ModelServiceConfigExternalModelConfig
* `model_provider_service` (string, required) - Resource name of the governed ModelProviderService that owns provider
  ...
  Each `{...}` component is capped at 255 characters individually
* `target` (ModelProviderServiceConfigModelTargetConfig, required) - Routing target for the destination: the provider-side model selected from
...
* `destinations` (list of ModelServiceConfigDestinationConfig, optional) - Ordered list of fallback destinations. Traversal is in list order; the
  attempt count is the length of the list. At most 5 are allowed

### ModelServiceConfigPayPerTokenConfig
* `model` (string, required) - Resource name of the UC model.
  Format: `models/{catalog}.{schema}.{model}`

### ModelServiceConfigProvisionedThroughputConfig
* `model_serving_endpoint` (string, required) - Name of the backing Model Serving endpoint serving the provisioned-
...
* `destinations` (list of ModelServiceConfigDestinationConfig, optional) - Primary routing destinations. At most 10 are allowed. At least one is
  ...
  `update_mask`
* `fallback` (ModelServiceConfigFallbackConfig, optional) - Fallback routing config, applied after primary destinations fail
* `first_token_timeout` (string, optional) - Timeout for the first token of a streaming response. If a destination does
...
* `traffic_splitting` (ModelServiceConfigRoutingConfigTrafficSplitting, optional) - Marker message selecting request-based traffic splitting. Traffic is
...
```sh
terraform import databricks_ai_gateway_model_service.this "name"
```